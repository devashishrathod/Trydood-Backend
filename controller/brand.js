...
exports.getAllBrand = async (req, res) => {
    ...
    try {
        if (id) {
            const result = await Brand.findById(id).populate("location").populate("gst").populate("workHours")
            if (result) {
                return res.status(200).json({ success: true, msg: "Brand details", result })
            }
            return res.status(404).json({ msg: "Brand not found", success: false })
        }
        const result = await Brand.find().sort({ createdAt: -1 }).populate("location").populate("gst").populate("workHours")
        if (result) {
            return res.status(200).json({ success: true, msg: "Brand details", result })
        }
        return res.status(404).json({ msg: "Brand not found", success: false })
...
exports.pagination = async (req, res) => {
    ...
    try {
        const result = await Brand.find().limit(limit).skip(skip).sort({ createdAt: -1 }).populate("location").populate("gst").populate("workHours")
        const totalDocuments = await Brand.countDocuments()
        const totalPages = Math.ceil(totalDocuments / limit);

        if (result) {
            return res.status(200).json({ success: true, msg: "Brand details", result, pagination: { totalDocuments, totalPages, currentPage: page, limit, } })
        }
        return res.status(404).json({ msg: "Brand not found", success: false })
    } catch (error) {
        console.log("error on getAllBrand: ", error);
...
exports.addBrand = async (req, res) => {
    ...
    try {
        ...
        const user = new User({ email: loginEmail, password: hashedPass, referCode })
        ...
        if (!checkGst) {
            const gst = new Gst({ companyName, gstNumber, zipCode, brand: brand._id, user: user?._id })
            await gst.save()
            user.gst = gst._id
            brand.gst = gst._id
        }

        const checkLogin = await User.findOne({ email: loginEmail, brand: brand._id, role: "vendor" })
        if (checkLogin) {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Brand added successfully", result })
        }
        return res.status(400).json({ msg: "Failed to save brand", success: false })
    } catch (error) {
